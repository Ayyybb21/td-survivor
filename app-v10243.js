...
function renderActivePickView(){
  // Switching between an owner's plays should only touch the Pick screen.
  // Re-rendering standings/history/admin here is unnecessary and can collide
  // with an in-flight pick save on slower mobile devices.
  renderHeader();
  renderPlayers();
}

function setActiveEntry(id){
  if(!state?.entries?.some(e=>String(e.id)===String(id)))return;
  localStorage.setItem("td_active_entry",id);
  selectedPlayer=null;
  closeModal();
  playerRenderLimit=60;
  renderActivePickView();
}
...
async function loadNFLShadowHero(){
  ...
  if(toggle&&diag){
    toggle.onclick=()=>{
      ...
      toggle.textContent=open?"🧪 Engine diagnostics":"🧪 Hide diagnostics";
    };
  }

  try{
    const r=await jsonp("nflLiveEngine",{
      week:String(state.league.week),
    ...
    if(g){
      ...
      if(td.dataAvailable){
        if(td.hit){
          tdHtml=`<span style="color:#43dc87;font-weight:950;">🏈 TOUCHDOWN DETECTED ✓</span>
            <div style="margin-top:2px;color:#84909b;font-size:7px;">
              Rush ${td.rushingTD||0} • Rec ${td.receivingTD||0}${td.returnTD?` • Return ${td.returnTD}`:""}
            </div>`;
        }else{
          tdHtml=`<span style="color:#f0b45d;font-weight:900;">TD NOT YET DETECTED</span>
            <div style="margin-top:2px;color:#84909b;font-size:7px;">Current BBS Week ${state.league.week} player-stat row = 0 qualifying TDs</div>`;
        }
      }else{
        tdHtml=`<span style="color:#8f99a4;font-weight:900;">WAITING FOR PLAYER STATS</span>
          <div style="margin-top:2px;color:#68727d;font-size:7px;">No 2026 Week ${state.league.week} player-stat row is available yet.</div>`;
      }

      box.innerHTML=`
        <div>${score}</div>
        <div style="margin-top:3px;color:${s==="live"?"#e7ebef":"#8f99a4"};">${detail}</div>
        <div style="margin-top:7px;padding-top:7px;border-top:1px solid #20252b;">
          <div style="color:#cfd5db;font-weight:850;">${pick.player}</div>
          <div style="margin-top:3px;">${tdHtml}</div>
        </div>`;
    }else{
      badge.textContent="NO GAME";
      badge.style.color="#8f99a4";
    ...
    if(toggle&&diag){
      ...
      diag.innerHTML=`
        <div style="color:#ff6a00;font-weight:950;letter-spacing:.6px;">LIVE ENGINE V1</div>
        <div style="margin-top:5px;">
          Player: ${r.player?.name||pick.player}<br>
          Team: ${r.player?.team||team}<br>
          GSIS ID: ${r.player?.gsisId||"not resolved"}<br>Resolver: ${r.player?.gsisId?"MATCHED":"NO SAFE MATCH"}<br>
          Player stats HTTP: ${r.stats?.http ?? "—"}<br>
          Week row available: ${r.stats?.available?"YES":"NO"}<br>
          Checked: ${r.checkedAt||"—"}
        </div>
        <div style="margin-top:6px;color:#7d8791;">
          SAFETY LOCK<br>
          Official result changed: ${safety.officialResultChanged?"YES":"NO"}<br>
          Entry status changed: ${safety.entryStatusChanged?"YES":"NO"}<br>
          Buyback changed: ${safety.buybackChanged?"YES":"NO"}
        </div>`;
    }
  }catch(err){
    badge.textContent="UNAVAILABLE";
    badge.style.color="#ff8a72";
    box.textContent="NFL Live Engine is temporarily unavailable.";

    if(diag){
      diag.innerHTML=`
        <div style="color:#ff8a72;font-weight:950;">LIVE ENGINE ERROR</div>
        <div style="margin-top:5px;color:#aab2bb;word-break:break-word;">${String(err?.message||err)}</div>`;
    }
    console.warn("NFL Live Engine",err);
  }
}
  ...
  hero.innerHTML=`
    ...
    <div id="nflShadowHero" style="margin-top:13px;padding:10px;border:1px solid #2d3339;border-radius:11px;background:#080a0d;">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
        <span style="font-size:7px;font-weight:950;letter-spacing:.75px;color:${brandOrange};">NFL LIVE • ENGINE V1</span>
      ...
      ` : ""}
      <div style="margin-top:5px;font-size:6.5px;color:#66717c;">Read-only test • official grading is still manual</div>
    </div>` : ""}
...
function renderLeagueHistory(selectedId=null,mode="history"){
  ...
  const currentWeek=Number(state?.league?.week||1);
  const leagueLocked=Boolean(state?.league?.locked);
  const myEntryIds=new Set((state?.entries||[]).map(e=>String(e.id)));
  const selectedIsMine=selected?.isMine===true || myEntryIds.has(String(selected?.id));
  const hideCurrentWeek=!leagueLocked && !selectedIsMine;
  const used=new Set();
  picks.forEach(p=>{
    if(p.playerId)used.add(String(p.playerId));
    if(p.player)used.add(slug(p.player));
  ...
  if(mode==="history"){
    const revealedRows=picks.map(p=>{
    ...
    // Always show the same neutral placeholder for another owner's current
    // week before lock. It does not disclose whether that owner submitted.
    const hiddenRow=hideCurrentWeek ? `
      <div class="row">
        <span class="week">WEEK ${currentWeek}</span>
        <span class="pick" style="color:#9aa3ad!important;">🔒 Pick hidden until deadline</span>
        <span class="rstatus" style="color:#9aa3ad!important;">HIDDEN</span>
      </div>` : "";

    body.innerHTML=revealedRows+hiddenRow || '<div class="muted">No picks submitted yet.</div>';
  }else{
    body.innerHTML=`
...
function openLeagueHistory(entryId){
  document.querySelectorAll(".navbtn").forEach(b=>b.classList.toggle("active",b.dataset.view==="leagueView"));
  ["pickView","sundayView","leagueView","historyView","chatView","adminView"].forEach(id=>{
    document.querySelector("#"+id)?.classList.toggle("hidden",id!=="leagueView");
  });

  renderStandings();
  renderLeagueHistory(entryId,"history");

  // History now lives below Standings in the same League screen. Previously
  // the selected play changed correctly, but the viewport stayed at the top,
  // making the tap appear to do nothing. Move the user directly to Scouting.
  requestAnimationFrame(()=>{
    const host=document.querySelector("#leagueHistoryHost");
    if(host){
...
function tdSimpleDeviceKey(endpoint){
...
async function syncPushStatusToLeague(sub,enabled){
  if(!OWNER_TOKEN||!sub?.endpoint)return;
  try{
    await jsonp("syncPushStatus",{
      token:OWNER_TOKEN,
      deviceKey:tdSimpleDeviceKey(sub.endpoint),
...
async function enableTDPush(){
  ...
  await syncPushStatusToLeague(sub,true);
  return sub;
}
async function disableTDPush(){const sub=await currentPushSubscription();if(sub){await syncPushStatusToLeague(sub,false);try{await fetch(PUSH_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"unsubscribe",endpoint:sub.endpoint})});}catch(_){}await sub.unsubscribe().catch(()=>{});}}
async function renderNotificationCard(){
  ...
  if(sub&&OWNER_TOKEN)syncPushStatusToLeague(sub,enabled);
...
function tdBuildPreviousWeekRecap(){
  const currentWeek=Number(state?.league?.week||1);
  const previousWeek=currentWeek-1;
  if(previousWeek<1)throw new Error("There is no completed week to recap yet.");

  const entries=state?.leagueEntries||state?.standings||[];
  const previousResults=[];
  entries.forEach(e=>{
    const pick=(e.picks||e.pickHistory||[]).find(p=>Number(p.week)===previousWeek);
    if(pick)previousResults.push({entry:e,pick});
  });

  const eliminated=previousResults.filter(x=>String(x.pick.result||"").trim().toLowerCase()==="no td");
  const buybackEligible=eliminated.filter(x=>!x.entry.buybackUsed).length;
  const aliveNow=(adminState?.entries||[]).filter(e=>e.status==="alive").length;
  const buybacksUsed=(adminState?.entries||[]).filter(e=>e.buybackUsed).length;
  const projectedPot=(adminState?.entries||[]).length*Number(state?.league?.entryFee||20)+buybacksUsed*Number(state?.league?.buybackFee||10);
  const deadline=String(state?.league?.deadline||adminState?.league?.deadline_label||"Deadline TBD");

  return [
    `🏈 WEEK ${previousWeek} COMPLETE`,
    "",
    `✅ Week ${previousWeek} is officially in the books.`,
    `🟢 ${aliveNow} play${aliveNow===1?"":"s"} advance to Week ${currentWeek}.`,
    `🔴 ${eliminated.length} play${eliminated.length===1?" was":"s were"} eliminated this week.`,
    eliminated.length ? `♻️ ${buybackEligible} of ${eliminated.length} eliminated play${eliminated.length===1?"":"s"} ${buybackEligible===1?"has":"have"} a buyback available for Week ${currentWeek}.` : "",
    projectedPot ? `💰 Projected pot: $${projectedPot}` : "",
    "",
    `🔓 WEEK ${currentWeek} IS NOW OPEN`,
    `⏰ Picks due: ${deadline}`,
    "",
    "Good luck 🫡"
  ].filter(Boolean).join("\n");
...
async function tdPostSystemWithRetry(body,attempts=3){
  let lastErr=null;
  for(let i=1;i<=attempts;i++){
    try{
      await postSystemMessage(body);
      return true;
    }catch(err){
      lastErr=err;
  ...
  throw lastErr||new Error("Chat post failed.");
}
...
function renderAdmin(){
  ...
  view.innerHTML=`
    ...
    <div class="admin-toolbar">
      ...
      <button id="recoverOwnerLoginAdmin">🔑 Recover Login</button>
      ...
      <button id="repostWeekRecapAdmin">📣 Post Week ${Math.max(Number(state.league.week||1)-1,1)} Recap</button>
      <button id="deadlineAdmin">⏰ Change Deadline</button>
      <button id="weekAdmin">📅 Manual Week</button>
      <button id="liveGradeAdmin">🏈 Live Grading</button>
      ...
      <button id="pushStatusAdmin">🔔 Notification Status</button>
      <button id="smartReminderAdmin">🤖 Automated Notifications: ${(adminState.autoNotifications?.master)?"ON":"OFF"}</button>
      <button id="resetSeasonAdmin">🧹 Reset Season</button>
      <button id="freshLeagueAdmin" style="border-color:#7a2634;color:#ffb0ba;">🔥 Start Fresh League</button>
  ...
  document.querySelector("#recoverOwnerLoginAdmin").onclick=()=>{
    const existing=document.querySelector("#recoverOwnerLoginPanel");
    if(existing){existing.remove();return;}

    const activeOwners=[...(adminState.owners||[])]
      .filter(o=>o.active!==false)
      .sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));

    if(!activeOwners.length){
      alert("There are no active owners to recover.");
      return;
    }

    const panel=document.createElement("div");
    panel.id="recoverOwnerLoginPanel";
    panel.style.cssText="margin-top:10px;padding:12px;border:1px solid #a83d00;border-radius:12px;background:#0c0e11;";

    panel.innerHTML=`
      <div style="font-size:9px;color:#ff7a1a;font-weight:950;letter-spacing:.9px;margin-bottom:4px;">
        🔑 RECOVER PRIVATE LOGIN
      </div>

      <div style="font-size:8px;color:#929ca7;line-height:1.45;margin-bottom:10px;">
        Use this when someone already joined but lost or closed their private owner link before saving TD Survivor to their Home Screen.
      </div>

      <label style="display:block;font-size:9px;color:#aeb8c5;margin-bottom:5px;">
        Choose person
      </label>

      <select id="recoverOwnerSelect"
        style="width:100%;background:#090b0e;color:#fff;border:1px solid #36404b;border-radius:10px;padding:11px;margin-bottom:9px;">
        <option value="">Select person…</option>
        ${activeOwners.map(o=>`<option value="${o.id}">${o.name}</option>`).join("")}
      </select>

      <button id="getPrivateLoginLink" class="primary" style="width:100%;">
        Get Private Login Link
      </button>

      <div id="privateLoginResult"
        style="display:none;margin-top:11px;padding-top:11px;border-top:1px solid #292e35;">

        <div id="privateLoginOwner"
          style="font-size:9px;font-weight:900;color:#fff;margin-bottom:6px;"></div>

        <div id="privateLoginUrl"
          style="font-size:8px;line-height:1.4;color:#aab3bd;background:#090b0e;border:1px solid #343941;border-radius:9px;padding:8px;word-break:break-all;"></div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:8px;">
          <button id="copyPrivateLogin"
            style="padding:10px;border-radius:9px;border:1px solid #a83d00;background:#15100d;color:#ff7a1a;font-weight:900;">
            📋 Copy Link
          </button>

          <button id="sharePrivateLogin"
            style="padding:10px;border-radius:9px;border:1px solid #a83d00;background:#ff5a00;color:#fff;font-weight:900;">
            📤 Share
          </button>
        </div>

        <div style="font-size:7.5px;color:#7f8994;line-height:1.45;margin-top:8px;">
          This opens that person's EXISTING account. It does not create a duplicate person or play.
        </div>
      </div>
    `;

    document.querySelector("#recoverOwnerLoginAdmin")
      .insertAdjacentElement("afterend",panel);

    let currentPrivateUrl="";
    let currentOwnerName="";

    document.querySelector("#getPrivateLoginLink").onclick=async()=>{
      const ownerId=document.querySelector("#recoverOwnerSelect").value;
      const owner=activeOwners.find(o=>String(o.id)===String(ownerId));
      ...
      const btn=document.querySelector("#getPrivateLoginLink");
      btn.disabled=true;
      btn.textContent="Getting link…";

      try{
        const data=await jsonp("ownerInvite",{
          adminToken:ADMIN_TOKEN,
          ownerId:owner.id
        });

        if(!data?.token)throw new Error("Private owner token was not returned.");

        const u=new URL(baseInviteUrl());
        u.searchParams.set("token",data.token);

        currentPrivateUrl=u.toString();
        currentOwnerName=data.name||owner.name;

        document.querySelector("#privateLoginOwner").textContent=
          `${currentOwnerName}'s private login`;

        document.querySelector("#privateLoginUrl").textContent=currentPrivateUrl;
        document.querySelector("#privateLoginResult").style.display="block";
      }catch(err){
        alert("Could not recover private login: "+err.message);
      }finally{
        btn.disabled=false;
        btn.textContent="Get Private Login Link";
      }
    };

    document.querySelector("#copyPrivateLogin").onclick=async()=>{
      if(!currentPrivateUrl)return;

      try{
        await copyTextFallback(currentPrivateUrl);

        const b=document.querySelector("#copyPrivateLogin");
        const old=b.textContent;
        b.textContent="✅ Copied";
        setTimeout(()=>b.textContent=old,1200);
      }catch(_){
        alert("Could not copy the private login link.");
      }
    };

    document.querySelector("#sharePrivateLogin").onclick=async()=>{
      if(!currentPrivateUrl)return;

      const text=
        `Here is your TD Survivor private login link. `+
        `This opens the account you already created:\n\n${currentPrivateUrl}`;

      if(navigator.share){
        try{
          await navigator.share({
            title:"TD Survivor Private Login",
            text:text
          });
          return;
        }catch(err){
      ...
      try{
        await copyTextFallback(text);
        alert("Private login message copied. Paste it into your text conversation.");
      }catch(_){
        alert(text);
      }
    };
  ...
  document.querySelector("#addEntryAdmin").onclick=()=>{
    const existing=document.querySelector("#addEntryPanel");
    if(existing){existing.remove();return;}
    ...
    if(!activeOwners.length){
      alert("There are no active owners available.");
      return;
    }

    const panel=document.createElement("div");
    panel.id="addEntryPanel";
    panel.style.cssText="margin-top:10px;padding:12px;border:1px solid #343941;border-radius:12px;background:#0b0e12;";

    panel.innerHTML=`
      <div style="font-size:9px;color:#ff6a00;font-weight:950;letter-spacing:.9px;margin-bottom:7px;">ADD PLAY</div>

      <label style="display:block;font-size:9px;color:#aeb8c5;margin-bottom:5px;">
        Choose owner
      </label>

      <select id="addEntryOwnerSelect"
        style="width:100%;background:#090b0e;color:#fff;border:1px solid #36404b;border-radius:10px;padding:11px;margin-bottom:9px;">
        <option value="">Select owner…</option>
        ${activeOwners.map(o=>`<option value="${o.id}">${o.name}</option>`).join("")}
      </select>

      <button id="confirmAddEntry"
        style="width:100%;padding:11px;border-radius:10px;border:1px solid #ff6a00;background:#2a1307;color:#fff;font-weight:950;">
        Add Play
      </button>
    `;

    document.querySelector("#addEntryAdmin").insertAdjacentElement("afterend",panel);

    document.querySelector("#confirmAddEntry").onclick=async()=>{
      const ownerId=document.querySelector("#addEntryOwnerSelect").value;
      const owner=activeOwners.find(o=>String(o.id)===String(ownerId));

      if(!owner){
        alert("Choose an owner first.");
        return;
      }

      if(!confirm(`Add another play for ${owner.name}?`))return;

      const btn=document.querySelector("#confirmAddEntry");
      btn.disabled=true;
      btn.textContent="Adding…";

      try{
        await adminPost("addEntry",{ownerId:owner.id});
        btn.textContent="✅ Added";
      }catch(err){
        btn.disabled=false;
        btn.textContent="Add Play";
        alert(err.message);
      }
    };
  };
  ...
  document.querySelector("#repostWeekRecapAdmin").onclick=async()=>{
    const btn=document.querySelector("#repostWeekRecapAdmin");
    const previousWeek=Number(state?.league?.week||1)-1;
    if(previousWeek<1){alert("There is no completed week to recap yet.");return;}
    if(!confirm(`Post the Week ${previousWeek} Complete recap to league chat now?\n\nThis does not change picks, grading, buybacks, or the current week.`))return;
    const original=btn.textContent;
    btn.disabled=true;
    btn.textContent=`Posting Week ${previousWeek} recap…`;
    try{
      const recap=tdBuildPreviousWeekRecap();
      await tdPostSystemWithRetry(recap,3);
      btn.textContent=`✅ Week ${previousWeek} recap posted`;
      alert(`Week ${previousWeek} Complete recap was posted to league chat.`);
    }catch(err){
      btn.disabled=false;
      btn.textContent=original;
      alert(`The Week ${previousWeek} recap did not post.\n\n${err.message}\n\nNothing in the league was changed. You can safely try this button again.`);
    }
  };
  ...
  document.querySelector("#advanceWeekAdmin").onclick=async()=>{
    ...
    try{
      ...
      try{
        ...
        // V10.24.3: make the recap durable enough for normal transient failures.
        // Retry with a fresh system proof each time, and never fail silently.
        await tdPostSystemWithRetry(recap,3);
      }catch(chatErr){
        console.warn("Week recap chat post failed",chatErr);
        alert(`Week ${currentWeek} advanced successfully, but its automatic chat recap did not post.\n\nUse Admin → Post Week ${currentWeek} Recap to retry it safely.\n\nNo league data was changed by the chat failure.`);
      }
  ...
  document.querySelector("#liveGradeAdmin").onclick=async()=>{
    document.querySelector("#liveGradePanel")?.remove();
    const btn=document.querySelector("#liveGradeAdmin");
    btn.disabled=true;
    const original=btn.textContent;
    btn.textContent="🏈 Loading Live Grading…";

    try{
      await loadGamedayBoard(true);
      const board=TD_GAMEDAY_BOARD;
      if(!board?.locked)throw new Error("Week picks must be locked before Live Grading is available.");

      const groups=board.groups||[];
      const panel=document.createElement("div");
      panel.id="liveGradePanel";
      panel.style.cssText="margin-top:10px;padding:12px;border:1px solid #256648;border-radius:12px;background:#09130f;";
      panel.innerHTML=`
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;">
          <div>
            <div style="font-size:9px;color:#31d07c;font-weight:950;letter-spacing:.8px;">🏈 LIVE GRADING • WEEK ${board.week}</div>
            <div style="font-size:8px;color:#8fa59a;margin-top:3px;">NFL Engine = evidence. Your grade = official result.</div>
          </div>
          <button id="closeLiveGrade" style="width:28px;height:28px;border-radius:50%;border:1px solid #3a3f46;background:#121417;color:#aeb6bf;">×</button>
        </div>
        <div id="liveGradeRows" style="margin-top:10px;">
          ${groups.map((g,i)=>{
            const meta=tdLiveStateMeta(g);
            const canTd=g.liveState==="safe";
            const canNoTd=g.liveState==="at_risk";
            return `
              <div class="tdLiveGradeRow" data-live-grade="${i}" style="padding:11px 0;border-top:1px solid #26302b;">
                <div style="display:flex;justify-content:space-between;gap:8px;">
                  <div>
                    <div style="font-size:11px;font-weight:950;">${chatEsc(g.player)} <span style="color:#7f8994;font-size:8px;">• ${g.count} ${g.count===1?"play":"plays"}</span></div>
                    <div style="font-size:8px;color:${meta.color};font-weight:900;margin-top:3px;">${meta.icon} ${meta.label}</div>
                    <div style="font-size:7.5px;color:#85908c;margin-top:3px;">${tdGameLine(g.engine?.game)}</div>
                    <div style="font-size:7.5px;color:#85908c;margin-top:3px;">Official: ${chatEsc(g.officialResult||"Pending")}</div>
                  </div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px;">
                  <button class="tdLiveGradeTd" data-player="${chatEsc(g.player)}" ${canTd?"":"disabled"} style="padding:8px;border-radius:9px;border:1px solid #256648;background:#10281e;color:#31d07c;font-size:8px;font-weight:950;opacity:${canTd?1:.35};">✅ GRADE TD</button>
                  <button class="tdLiveGradeNoTd" data-player="${chatEsc(g.player)}" ${canNoTd?"":"disabled"} style="padding:8px;border-radius:9px;border:1px solid #68343a;background:#351a20;color:#ff8993;font-size:8px;font-weight:950;opacity:${canNoTd?1:.35};">❌ GRADE NO TD</button>
                </div>
                ${!canTd&&!canNoTd?`<div style="font-size:7px;color:#68727d;margin-top:6px;">Grading buttons unlock only when the engine detects a TD or a game is final with no TD.</div>`:""}
              </div>`;
          }).join("")}
        </div>
        <div id="liveGradeStatus" style="font-size:8px;color:#8fa59a;margin-top:8px;"></div>`;

      document.querySelector("#liveGradeAdmin").insertAdjacentElement("afterend",panel);
      panel.querySelector("#closeLiveGrade").onclick=()=>panel.remove();

      const grade=async(player,scored)=>{
        const label=scored?"TD":"NO TD";
        if(!confirm(`Officially grade ${player} as ${label}? This changes every Week ${board.week} play using that player.`))return;
        const status=panel.querySelector("#liveGradeStatus");
        status.textContent=`Saving ${player} as ${label}…`;
        try{
          await safeCommissionerPost("gradePlayer",{adminToken:ADMIN_TOKEN,playerName:player,scored});
          await new Promise(r=>setTimeout(r,900));
          await Promise.all([refreshLive(),loadAdminState()]);
          TD_GAMEDAY_BOARD=null;
          await loadGamedayBoard(true);
          status.textContent=`✅ ${player} officially graded ${label}.`;
          renderAdmin();
        }catch(err){
          status.textContent="";
          alert("Live Grading: "+err.message);
        }
      };

      panel.querySelectorAll(".tdLiveGradeTd").forEach(x=>x.onclick=()=>grade(x.dataset.player,true));
      panel.querySelectorAll(".tdLiveGradeNoTd").forEach(x=>x.onclick=()=>grade(x.dataset.player,false));
    }catch(err){
      alert("Live Grading: "+err.message);
    }finally{
      btn.disabled=false;
...
Get those picks in!`;
  ...
  document.querySelector("#pushAnnouncementAdmin").onclick=async()=>{
    ...
    try{
      const r=await jsonp("pushAnnouncement",{adminToken:ADMIN_TOKEN,message});
      const names=Array.isArray(r.notificationOwners)?r.notificationOwners:[];
      alert(
        `Announcement sent to ${r.sent||0} device${Number(r.sent||0)===1?"":"s"}.`+
        (names.length
          ? `\n\nOwners currently registered for notifications (${names.length}):\n• ${names.join("\n• ")}`
          : "\n\nNo current owner notification registrations were found.")
      );
    }catch(err){alert("Push announcement failed: "+err.message)}
  };


  document.querySelector("#pushStatusAdmin").onclick=async()=>{
    const existing=document.querySelector("#pushStatusPanel");
    if(existing){existing.remove();return;}

    const btn=document.querySelector("#pushStatusAdmin");
    btn.disabled=true;
    const oldText=btn.textContent;
    btn.textContent="Loading…";

    try{
      const r=await jsonp("pushStatus",{adminToken:ADMIN_TOKEN});
      const owners=Array.isArray(r.owners)?r.owners:[];

      const panel=document.createElement("div");
      panel.id="pushStatusPanel";
      panel.style.cssText="margin-top:10px;padding:12px;border:1px solid #34404b;border-radius:12px;background:#0a0d10;";

      const on=owners.filter(o=>o.enabled);
      const off=owners.filter(o=>!o.enabled);

      panel.innerHTML=`
        <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;">
          <div>
            <div style="font-size:9px;color:#ff7a1a;font-weight:950;letter-spacing:.8px;">🔔 NOTIFICATION STATUS</div>
            <div style="font-size:8px;color:#8f99a4;margin-top:3px;">
              ${on.length}/${owners.length} owners registered • ${Number(r.devices||0)} device${Number(r.devices||0)===1?"":"s"}
            </div>
          </div>
          <button id="closePushStatus" style="padding:5px 7px;border-radius:8px;border:1px solid #343941;background:#11151a;color:#aab2bb;">✕</button>
        </div>

        <div style="margin-top:10px;">
          ${owners.map(o=>`
            <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;padding:8px 0;border-top:1px solid #1f252b;">
              <div>
                <div style="font-size:9px;font-weight:900;color:#fff;">${o.name}</div>
                <div style="font-size:7px;color:#7d8791;margin-top:2px;">
                  ${o.enabled
                    ? `${o.devices} registered device${Number(o.devices)===1?"":"s"}`
                    : "No registered notification device"}
                </div>
              </div>
              <div style="font-size:8px;font-weight:950;color:${o.enabled?"#38d77a":"#ff6f7f"};">
                ${o.enabled?"ON":"OFF"}
              </div>
            </div>
          `).join("")}
        ...
        <div style="font-size:7px;color:#6f7983;line-height:1.45;margin-top:9px;">
          Status is based on devices registered for TD Survivor push notifications. A successful send count is still measured by delivered devices, so it can differ from owner count.
        </div>
      `;

      btn.insertAdjacentElement("afterend",panel);
      document.querySelector("#closePushStatus").onclick=()=>panel.remove();
    }catch(err){
      alert("Could not load notification status: "+err.message);
    }finally{
      btn.disabled=false;
  ...
  document.querySelector("#smartReminderAdmin").onclick=()=>{
    const existing=document.querySelector("#autoNotificationsPanel");
    if(existing){existing.remove();return;}

    const cfg=adminState.autoNotifications||{};
    const activeOwners=[...(adminState.owners||[])]
      .filter(o=>o.active!==false)
      .sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));

    const panel=document.createElement("div");
    panel.id="autoNotificationsPanel";
    panel.style.cssText="margin-top:10px;padding:14px;border:1px solid #3b424b;border-radius:14px;background:#0a0d10;";

    const row=(id,title,desc,on)=>`
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;padding:10px 0;border-top:1px solid #1d2329;">
        <div style="min-width:0;">
          <div style="font-size:9px;color:#fff;font-weight:900;">${title}</div>
          <div style="font-size:7.5px;color:#808b96;line-height:1.4;margin-top:2px;">${desc}</div>
        </div>
        <button class="autoNotifyToggle" data-id="${id}" data-on="${on?"1":"0"}"
          style="flex:0 0 57px;padding:8px 7px;border-radius:9px;border:1px solid ${on?"#256846":"#4b3338"};background:${on?"#0d2b1c":"#1e1013"};color:${on?"#42dc88":"#ff7583"};font-weight:950;">
          ${on?"ON":"OFF"}
        </button>
      </div>`;
    ...
    panel.innerHTML=`
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
        <div>
          <div style="font-size:9px;color:#ff6a00;font-weight:950;letter-spacing:.9px;">🤖 AUTOMATED NOTIFICATIONS</div>
          <div style="font-size:8px;color:#8c96a1;line-height:1.45;margin-top:4px;">
            Choose which alerts TD Survivor may send automatically. Manual push buttons remain available.
          </div>
        </div>
        <button id="closeAutoNotifications" style="padding:5px 7px;border-radius:8px;border:1px solid #343941;background:#11151a;color:#aab2bb;">✕</button>
      </div>

      <div style="margin-top:12px;padding:11px;border-radius:11px;border:1px solid ${cfg.master?"#285d44":"#5d3339"};background:${cfg.master?"#0b1c14":"#180d10"};">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;">
          <div>
            <div style="font-size:10px;color:#fff;font-weight:950;">MASTER SWITCH</div>
            <div style="font-size:7.5px;color:#89939e;margin-top:2px;">${cfg.master?"Automatic alerts are allowed.":"All automatic alerts are currently blocked."}</div>
          </div>
          <button id="autoNotifyMaster" data-on="${cfg.master?"1":"0"}"
            style="min-width:68px;padding:9px;border-radius:10px;border:1px solid ${cfg.master?"#256846":"#6b3039"};background:${cfg.master?"#0d2b1c":"#251014"};color:${cfg.master?"#42dc88":"#ff7583"};font-weight:950;">
            ${cfg.master?"ON":"OFF"}
          </button>
        </div>
      </div>

      <div style="font-size:8px;color:${cfg.triggerInstalled?"#42dc88":"#f0b45d"};margin-top:9px;line-height:1.4;">
        ${cfg.triggerInstalled
          ?"✅ Scheduler installed • reminders + automatic deadline lock active."
          :"⚠️ Scheduler is NOT installed yet. Reminders + automatic deadline lock require a one-time Apps Script install."}
      </div>

      <div style="margin-top:10px;">
        <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:.7px;margin-bottom:3px;">MISSING PICK REMINDERS</div>
        ${row("missing24","24 hours before deadline","Only owners who still have missing ALIVE plays.",cfg.missing24!==false)}
        ${row("missing3","3 hours before deadline","Only owners who still owe a pick.",cfg.missing3!==false)}
        ${row("missing30","30 minutes before deadline","Final automated warning to owners still missing picks.",cfg.missing30!==false)}

        <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;padding:10px 0;border-top:1px solid #1d2329;">
          <div style="min-width:0;">
            <div style="font-size:9px;color:#fff;font-weight:900;">Automatic deadline lock</div>
            <div style="font-size:7.5px;color:#808b96;line-height:1.4;margin-top:2px;">
              At the configured deadline, picks automatically lock and reveal. Expected within about 0–60 seconds.
            </div>
          </div>
          <div style="flex:0 0 57px;padding:8px 7px;border-radius:9px;border:1px solid #256846;background:#0d2b1c;color:#42dc88;font-weight:950;text-align:center;">ON</div>
        </div>

        <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:.7px;margin:13px 0 3px;">LEAGUE EVENTS</div>
        ${row("picksLocked","Picks locked / revealed","League-wide alert when you lock the week.",cfg.picksLocked!==false)}
        ${row("newWeek","New week opened","Alerts surviving owners after you advance the week.",cfg.newWeek!==false)}
        ${row("survived","Pick graded — survived","Owner alert when their player is graded TD.",cfg.survived!==false)}
        ${row("eliminated","Pick graded — eliminated","Owner alert when their player is graded No TD.",cfg.eliminated!==false)}
        ${row("buyback","Buyback available","Alerts eligible owners when the buyback opens.",cfg.buyback!==false)}
        ${row("deadlineChanged","Deadline changed","League-wide alert when you change the deadline.",cfg.deadlineChanged!==false)}

        <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:.7px;margin:13px 0 3px;">COMMISSIONER</div>
        ${row("commissioner","Missing-pick commissioner alert","Private alert near the deadline when owners are still missing picks.",cfg.commissioner===true)}

        <label style="display:block;font-size:7.5px;color:#87919c;margin-top:9px;margin-bottom:4px;">Commissioner notification account</label>
        <select id="autoNotifyCommissionerOwner"
          style="width:100%;background:#080b0e;color:#fff;border:1px solid #343b43;border-radius:9px;padding:9px;">
          <option value="">Select owner…</option>
          ${activeOwners.map(o=>`<option value="${o.id}" ${String(cfg.commissionerOwnerId||"")===String(o.id)?"selected":""}>${o.name}</option>`).join("")}
        </select>
      </div>

      <div id="autoNotifySaveState" style="font-size:7.5px;color:#7e8994;margin-top:10px;">Changes save automatically.</div>
    `;

    document.querySelector("#smartReminderAdmin").insertAdjacentElement("afterend",panel);

    const close=panel.querySelector("#closeAutoNotifications");
    if(close)close.onclick=()=>panel.remove();

    const save=async(patch)=>{
      const state=panel.querySelector("#autoNotifySaveState");
      if(state){state.textContent="Saving…";state.style.color="#f0b45d";}
      try{
        await post("setAutoNotifications",{adminToken:ADMIN_TOKEN,...patch});
        if(!adminState.autoNotifications)adminState.autoNotifications={};
        Object.assign(adminState.autoNotifications,patch);
        if(state){state.textContent="✅ Saved";state.style.color="#42dc88";}
        setTimeout(()=>{if(state){state.textContent="Changes save automatically.";state.style.color="#7e8994";}},1000);
      }catch(err){
        if(state){state.textContent="⚠️ Could not save";state.style.color="#ff7583";}
        alert("Automated notification setting failed: "+err.message);
      }
    };

    const master=panel.querySelector("#autoNotifyMaster");
    master.onclick=async()=>{
      const now=master.dataset.on!=="1";
      master.dataset.on=now?"1":"0";
      master.textContent=now?"ON":"OFF";
      master.style.color=now?"#42dc88":"#ff7583";
      master.style.background=now?"#0d2b1c":"#251014";
      await save({master:now});
    };

    panel.querySelectorAll(".autoNotifyToggle").forEach(btn=>{
      btn.onclick=async()=>{
        const id=btn.dataset.id;
        const now=btn.dataset.on!=="1";
        btn.dataset.on=now?"1":"0";
        btn.textContent=now?"ON":"OFF";
        btn.style.color=now?"#42dc88":"#ff7583";
        btn.style.background=now?"#0d2b1c":"#1e1013";
        await save({[id]:now});
      };
    });

    panel.querySelector("#autoNotifyCommissionerOwner").onchange=async(e)=>{
      await save({commissionerOwnerId:e.target.value});
    };
  };


  document.querySelector("#freshLeagueAdmin").onclick=()=>{
    const existing=document.querySelector("#freshLeaguePanel");
    if(existing){existing.remove();return;}

    const activeOwners=[...(adminState.owners||[])].filter(o=>o.active!==false);
    const panel=document.createElement("div");
    panel.id="freshLeaguePanel";
    panel.style.cssText="margin-top:10px;padding:14px;border:1px solid #7b2936;border-radius:12px;background:#1b0d12;";

    panel.innerHTML=`
      <div style="font-size:9px;color:#ff7f8f;font-weight:950;letter-spacing:.9px;margin-bottom:7px;">🔥 ONE-TIME LAUNCH RESET</div>
      <div style="font-size:10px;color:#d5dbe3;line-height:1.5;margin-bottom:11px;">
        This creates a truly fresh league. It keeps ONLY the commissioner account you choose below and permanently removes every other person, play, pick, result, and old participant invite link.
      </div>

      <div style="padding:9px;border:1px solid #4d2530;border-radius:9px;background:#12090d;font-size:8.5px;color:#b9c1cc;line-height:1.45;margin-bottom:11px;">
        PRESERVED: app/backend, admin token, spreadsheet connection, BBS key, push system, selected commissioner's private account.<br>
        DELETED: every other owner, every old play, all Picks/history/results.<br>
        ALSO: old league invite is invalidated and a fresh invite token is created.
      </div>

      <label style="display:block;font-size:9px;color:#aeb8c5;margin-bottom:4px;">Commissioner account to KEEP</label>
      <select id="freshKeepOwner"
        style="width:100%;background:#07111f;color:#fff;border:1px solid #33455d;border-radius:10px;padding:10px;margin-bottom:9px;">
        ${activeOwners.map(o=>`<option value="${o.id}">${o.name}</option>`).join("")}
      </select>

      <label style="display:block;font-size:9px;color:#aeb8c5;margin-bottom:4px;">Starting week</label>
      <input id="freshStartWeek" type="number" min="1" max="25" value="1"
        style="width:100%;background:#07111f;color:#fff;border:1px solid #33455d;border-radius:10px;padding:10px;margin-bottom:9px;">

      <label style="display:block;font-size:9px;color:#aeb8c5;margin-bottom:4px;">Week 1 pick deadline</label>
      <input id="freshDeadline" value="${state.league.deadline}"
        style="width:100%;background:#07111f;color:#fff;border:1px solid #33455d;border-radius:10px;padding:10px;margin-bottom:11px;">

      <label style="display:flex;gap:8px;align-items:flex-start;font-size:9px;color:#c8d0da;line-height:1.4;margin-bottom:10px;">
        <input id="freshConfirmBox" type="checkbox">
        <span>I understand this permanently deletes every participant except the commissioner account selected above.</span>
      </label>

      <button id="runFreshLeague" class="primary" style="background:#8e2133;">🔥 START FRESH LEAGUE</button>
      <div style="font-size:8px;color:#8f9aa8;margin-top:8px;line-height:1.4;">
        After this succeeds, do not use test picks. This becomes your real 2026 production league.
      </div>
    `;

    document.querySelector("#freshLeagueAdmin").insertAdjacentElement("afterend",panel);

    document.querySelector("#runFreshLeague").onclick=async()=>{
      if(!document.querySelector("#freshConfirmBox").checked){
        alert("Check the confirmation box first.");
        return;
      }

      const keepOwnerId=document.querySelector("#freshKeepOwner").value;
      const keepOwner=activeOwners.find(o=>String(o.id)===String(keepOwnerId));
      const startWeek=Number(document.querySelector("#freshStartWeek").value||1);
      const deadlineLabel=document.querySelector("#freshDeadline").value.trim();

      const warning=
        `START A COMPLETELY FRESH TD SURVIVOR LEAGUE?\n\n`+
        `KEEP:\n• ${keepOwner?.name||"Selected commissioner"} only\n\n`+
        `PERMANENTLY DELETE:\n`+
        `• Every other participant\n`+
        `• Every old play/entry\n`+
        `• ALL picks/history/results\n`+
        `• Old participant invite links\n`+
        `• Old general league invite\n\n`+
        `CREATE:\n`+
        `• One clean ALIVE play for ${keepOwner?.name||"commissioner"}\n`+
        `• Week ${startWeek}, picks OPEN\n`+
        `• $0 starting pot / unpaid entry\n`+
        `• Fresh league invite token\n\n`+
        `This cannot be undone from the app.`;

      if(!confirm(warning))return;

      const phrase=prompt('Type START FRESH exactly to confirm:');
      if(phrase!=="START FRESH"){
        alert("Fresh-league reset cancelled.");
        return;
      }

      const btn=document.querySelector("#runFreshLeague");
      btn.disabled=true;
      btn.textContent="Creating fresh league…";

      try{
        await post("freshLeagueReset",{
          adminToken:ADMIN_TOKEN,
          keepOwnerId,
          startWeek,
          deadlineLabel,
          confirmPhrase:"START FRESH"
        });

        // POST uses no-cors by design, so the browser cannot read the Apps
        // Script response body. Treat a completed fetch as submitted, then
        // reload and verify the authoritative league state from bootstrap/adminState.
        btn.textContent="✅ Reset submitted";
        alert(
          `Fresh-league reset submitted ✅\n\n`+
          `The app will reload now and verify the actual league state.\n\n`+
          `Expected after reload:\n`+
          `• Only your commissioner account\n`+
          `• 1 play / 1 alive\n`+
          `• Week ${startWeek}\n`+
          `• No old picks/history\n\n`+
          `Do NOT press Start Fresh League again unless the old participants are still present after the reload.`
        );
        setTimeout(()=>window.location.reload(),700);
      }catch(err){
        btn.disabled=false;
        btn.textContent="🔥 START FRESH LEAGUE";
        alert("Fresh-league reset failed: "+err.message);
      }
    };
  };
...
function openModal(id,name){
  ...
  // Always reset the persistent Confirm button when opening a new player modal.
  // The modal DOM is reused between Play 1 / Play 2, so a disabled state from
  // a previous submission must never carry forward.
...
function ensureSundayAndLeagueUI(){
  const pickView=document.querySelector("#pickView");
  const standingsView=document.querySelector("#standingsView");
  const historyView=document.querySelector("#historyView");
  const adminView=document.querySelector("#adminView");
  if(!pickView||!standingsView||!historyView||!adminView)return;

  // Combine the old Standings + History destinations into one League screen.
  standingsView.id="leagueView";
  standingsView.classList.add("hidden");
  historyView.classList.add("hidden");

  let historyHost=document.querySelector("#leagueHistoryHost");
  if(!historyHost){
    historyHost=document.createElement("div");
    historyHost.id="leagueHistoryHost";
    historyHost.style.cssText="margin-top:14px;padding-top:14px;border-top:1px solid #252a30;";
    const historyInner=document.querySelector("#history");
    if(historyInner){
      historyHost.appendChild(historyInner);
      standingsView.appendChild(historyHost);
    }
  }

  // Repurpose old Standings nav button as League and remove old History button.
  const navBtns=[...document.querySelectorAll(".navbtn")];
  const standingsBtn=navBtns.find(b=>b.dataset.view==="standingsView");
  const historyBtn=navBtns.find(b=>b.dataset.view==="historyView");
  if(standingsBtn){
    standingsBtn.dataset.view="leagueView";
    standingsBtn.textContent="🏆 League";
  }
  historyBtn?.remove();

  // Sunday is second from the left: Pick | Sunday | League | Chat | Admin.
  if(!document.querySelector('.navbtn[data-view="sundayView"]')){
    const sundayBtn=document.createElement("button");
    sundayBtn.className="navbtn";
    sundayBtn.dataset.view="sundayView";
    sundayBtn.textContent="🏈 Gameday";
    const pickBtn=[...document.querySelectorAll(".navbtn")].find(b=>b.dataset.view==="pickView");
    pickBtn?.insertAdjacentElement("afterend",sundayBtn);
  }

  if(!document.querySelector("#sundayView")){
    const sunday=document.createElement("section");
    sunday.id="sundayView";
    sunday.className="hidden";
    sunday.style.cssText="padding-bottom:82px;";
    standingsView.parentNode.insertBefore(sunday,standingsView);
  }

  const nav=adminView.closest("body")?.querySelector(".navbtn")?.parentNode || adminView.parentNode?.querySelector(".navbtn")?.parentNode;
  const actualNav=[...document.querySelectorAll(".navbtn")][0]?.parentNode;
  if(actualNav){
    actualNav.style.display="grid";
    actualNav.style.gridTemplateColumns="repeat(5,1fr)";
  }
}

function renderLeague(){
  const league=document.querySelector("#leagueView");
  if(!league)return;
  renderStandings();
  renderLeagueHistory();
}
...
let TD_GAMEDAY_BOARD=null;
let TD_GAMEDAY_LOADING=false;
let TD_GAMEDAY_LAST_FETCH=0;
let TD_GAMEDAY_REFRESH_TIMER=null;

function tdGamedayTeamMap(){
  const map={};
  const names=new Set((state?.pickBreakdown?.players||[]).map(x=>String(x.player||"")));
  (state?.standings||[]).forEach(e=>{if(e.currentPick)names.add(String(e.currentPick));});
  names.forEach(name=>{
    const p=PLAYERS.find(x=>slug(x.name)===slug(name));
    if(p?.team){
      map[name]=String(p.team).toUpperCase();
      if(p.id)map[String(p.id)]=String(p.team).toUpperCase();
    }
  });
  return map;
}

async function loadGamedayBoard(force=false){
  if(!state?.league?.locked||TD_GAMEDAY_LOADING)return TD_GAMEDAY_BOARD;

  // V10.24.1: the original Gameday request could run before PLAYERS finished
  // loading, producing an empty team map for every league pick.
  if(!PLAYERS.length){
    try{await loadPlayerPool();}catch(_){}
  }
  if(!force && TD_GAMEDAY_BOARD && Date.now()-TD_GAMEDAY_LAST_FETCH<45000)return TD_GAMEDAY_BOARD;

  TD_GAMEDAY_LOADING=true;
  try{
    const extra={
      token:OWNER_TOKEN,
      teams:JSON.stringify(tdGamedayTeamMap())
    };
    if(ADMIN_TOKEN)extra.adminToken=ADMIN_TOKEN;
    TD_GAMEDAY_BOARD=await jsonp("nflGamedayBoard",extra);
    TD_GAMEDAY_LAST_FETCH=Date.now();
    return TD_GAMEDAY_BOARD;
  }catch(err){
    console.warn("Gameday board refresh failed",err);
    return TD_GAMEDAY_BOARD;
  }finally{
    TD_GAMEDAY_LOADING=false;
  }
}

function tdGameLine(game){
  if(!game)return "Game data pending";
  const state=String(game.state||"scheduled").toLowerCase();
  ...
  const matchup=hasScore
    ...
    : `${game.away} @ ${game.home}`;
  if(state==="live"){
    return `${matchup} • ${[game.periodLabel,game.clock].filter(Boolean).join(" • ")||"LIVE"}`;
  }
  if(state==="final")return `${matchup} • FINAL`;
  return `${game.away} @ ${game.home} • ${game.kickoffLabel||"Scheduled"}`;
}

function tdLiveStateMeta(group){
  const state=group.liveState;
  if(state==="safe")return {icon:"🔥",label:"TOUCHDOWN — SAFE",color:"#31d07c",border:"#256648",bg:"#0d2118"};
  if(state==="live")return {icon:"⏱",label:"WAITING FOR TD",color:"#f5c542",border:"#6a5225",bg:"#1c180e"};
  if(state==="at_risk")return {icon:"🔴",label:"FINAL — NO TD DETECTED",color:"#ff6673",border:"#68343a",bg:"#1b1115"};
  if(state==="upcoming")return {icon:"🕒",label:"UPCOMING",color:"#aab3bd",border:"#343941",bg:"#0b0d10"};
  return {icon:"⚪",label:"ENGINE DATA PENDING",color:"#8f99a4",border:"#343941",bg:"#0b0d10"};
}

function tdRenderGamedayLive(board){
  if(!board?.locked)return "";

  const s=board.summary||{};
  const groups=board.groups||[];
  const safeGroups=groups.filter(g=>g.liveState==="safe");

  const feed=safeGroups.length ? `
    <div class="card" style="margin-top:12px;padding:14px;border:1px solid #256648;background:#0b120f;">
      <div style="font-size:8px;color:#31d07c;font-weight:950;letter-spacing:1px;">🔥 TD FEED</div>
      <div style="margin-top:7px;">
        ${safeGroups.map(g=>`
          <div style="padding:8px 0;border-bottom:1px solid #203129;">
            <div style="font-size:11px;font-weight:950;">🏈 ${chatEsc(g.player)} — TD detected</div>
            <div style="font-size:8px;color:#8fa59a;margin-top:3px;">🎯 ${g.count} ${g.count===1?"entry":"entries"} hit • ${tdGameLine(g.engine?.game)}</div>
          </div>`).join("")}
      </div>
      <div style="font-size:7px;color:#66756d;margin-top:7px;">Feed shows engine detections, not official grading timestamps.</div>
    </div>` : "";

  const cards=groups.map((g,i)=>{
    const meta=tdLiveStateMeta(g);
    const official=String(g.officialResult||"Pending");
    const officialColor=official==="TD"?"#31d07c":official==="No TD"?"#ff6673":"#8f99a4";
    return `
      <div class="card tdGamedayGroup" data-gameday-group="${i}" style="margin-top:10px;padding:14px;border:1px solid ${meta.border};background:${meta.bg};cursor:pointer;">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
          <div style="min-width:0;">
            <div style="font-size:8px;color:${meta.color};font-weight:950;letter-spacing:.8px;">${meta.icon} ${meta.label}</div>
            <div style="font-size:15px;font-weight:950;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${chatEsc(g.player)}</div>
            <div style="font-size:8px;color:#9aa3ad;margin-top:4px;">${g.engine?.game?tdGameLine(g.engine.game):(g.engineError?chatEsc(g.engineError):"Game data pending")}</div>
          </div>
          <div style="text-align:right;flex:0 0 auto;">
            <div style="font-size:19px;font-weight:950;color:${meta.color};">${g.count}</div>
            <div style="font-size:7px;color:#7f8994;">${g.count===1?"ENTRY":"ENTRIES"}</div>
          </div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;padding-top:9px;border-top:1px solid #ffffff12;">
          <div style="font-size:7px;color:#7f8994;">OFFICIAL RESULT</div>
          <div style="font-size:8px;font-weight:950;color:${officialColor};">${official==="Pending"?"Pending commissioner grading":official}</div>
        </div>
        <div class="tdGamedayEntries" style="display:none;margin-top:9px;padding-top:8px;border-top:1px solid #ffffff12;">
          ${(g.entries||[]).map(e=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:5px 0;font-size:8px;"><span style="font-weight:900;">${chatEsc(e.label)}</span><span style="color:#8f99a4;">${chatEsc(e.officialResult||"Pending")}</span></div>`).join("")}
        </div>
        <div style="font-size:7px;color:#67717d;margin-top:8px;">Tap to ${g.count===1?"view play":"view all "+g.count+" plays"}</div>
      </div>`;
  }).join("");

  const checked=board.checkedAt ? new Date(board.checkedAt).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}) : "";

  return `
    <div class="card" style="margin-top:12px;padding:15px;border:1px solid #343941;background:#080a0c;">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
        <div>
          <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:1px;">🏈 WEEK ${board.week} — LIVE</div>
          <div style="font-size:17px;font-weight:950;margin-top:3px;">${Number(s.total||0)} PLAYS THIS WEEK</div>
        </div>
        <button id="tdRefreshGameday" style="padding:7px 9px;border:1px solid #343941;border-radius:9px;background:#111317;color:#aeb6bf;font-size:8px;font-weight:900;">↻ Refresh</button>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:12px;">
        <div style="padding:10px;border:1px solid #256648;border-radius:10px;background:#0d2118;"><div style="font-size:17px;font-weight:950;color:#31d07c;">${Number(s.safe||0)}</div><div style="font-size:7px;color:#83aa95;margin-top:2px;">🟢 SAFE • TD DETECTED</div></div>
        <div style="padding:10px;border:1px solid #6a5225;border-radius:10px;background:#1c180e;"><div style="font-size:17px;font-weight:950;color:#f5c542;">${Number(s.live||0)}</div><div style="font-size:7px;color:#b7a56f;margin-top:2px;">🟠 LIVE • WAITING</div></div>
        <div style="padding:10px;border:1px solid #68343a;border-radius:10px;background:#1b1115;"><div style="font-size:17px;font-weight:950;color:#ff6673;">${Number(s.atRisk||0)}</div><div style="font-size:7px;color:#b58d92;margin-top:2px;">🔴 FINAL • NO TD</div></div>
        <div style="padding:10px;border:1px solid #343941;border-radius:10px;background:#0b0d10;"><div style="font-size:17px;font-weight:950;color:#aeb6bf;">${Number(s.upcoming||0)+Number(s.unknown||0)}</div><div style="font-size:7px;color:#808995;margin-top:2px;">⚪ UPCOMING / PENDING</div></div>
      </div>
      <div style="font-size:7px;color:#67717d;line-height:1.45;margin-top:9px;">Read-only NFL Engine. Official survivor results change only when the commissioner grades them.${checked?` • Checked ${checked}`:""}</div>
    </div>
    ${feed}
    <div style="margin-top:14px;"><div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:1px;padding:0 2px;">LIVE PLAYER BOARD</div>${cards||'<div class="muted" style="padding:15px 2px;">Waiting for Gameday data…</div>'}</div>`;
}

function tdWireGameday(){
  const el=document.querySelector("#sundayView");
  if(!el)return;
  el.querySelector("#tdRefreshGameday")?.addEventListener("click",async e=>{
    e.currentTarget.disabled=true;
    e.currentTarget.textContent="Refreshing…";
    await loadGamedayBoard(true);
    renderSunday();
  });
  el.querySelectorAll(".tdGamedayGroup").forEach(card=>{
    card.onclick=()=>{
      const inner=card.querySelector(".tdGamedayEntries");
      if(inner)inner.style.display=inner.style.display==="none"?"block":"none";
    };
  });
}

function tdStartGamedayAutoRefresh(){
  clearInterval(TD_GAMEDAY_REFRESH_TIMER);
  TD_GAMEDAY_REFRESH_TIMER=setInterval(async()=>{
    if(document.querySelector("#sundayView")?.classList.contains("hidden"))return;
    await loadGamedayBoard(true);
    renderSunday();
  },60000);
}
...
function renderSunday(){
  const el=document.querySelector("#sundayView");
  if(!el||!state)return;
  const week=Number(state.league.week||1);

  if(!state.league.locked){
    el.innerHTML=`
      <div style="padding:18px 4px 10px;">
        <div class="eyebrow">GAME DAY</div>
        <div style="font-size:24px;font-weight:950;letter-spacing:-.7px;margin-top:3px;">🏈 Gameday</div>
      </div>
      <div class="card" style="padding:18px;border:1px solid #343941;background:#0b0d10;">
        <div style="font-size:9px;color:#ff7a1a;font-weight:950;letter-spacing:1px;">WEEK ${week} PLAYER PICKS</div>
        <div style="font-size:18px;font-weight:950;margin-top:7px;">🔒 Reveals when picks lock</div>
        <div style="font-size:9px;color:#8f99a4;line-height:1.5;margin-top:7px;">The league-wide player distribution stays private until the deadline. No pick counts are sent to participants before lock.</div>
      </div>`;
    return;
  }

  const bd=state.pickBreakdown||{players:[],total:0};
  // V10.24.2: once the commissioner has officially graded a selected player,
  // mirror that result in the Player Picks leaderboard. Engine-only detections
  // remain neutral until official grading is recorded.
  const officialByPlayer=new Map(
    (TD_GAMEDAY_BOARD?.groups||[]).map(g=>[slug(g.player),String(g.officialResult||"Pending")])
  );
  const rows=(bd.players||[]).map((x,i)=>{
    const official=officialByPlayer.get(slug(x.player))||"Pending";
    const pickColor=official==="TD"?"#31d07c":official==="No TD"?"#ff6673":"#f7f8fa";
    const countColor=official==="TD"?"#31d07c":official==="No TD"?"#ff6673":"#ff7a1a";
    return `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid #23272d;">
      <div style="display:flex;align-items:center;gap:9px;min-width:0;">
        <span style="width:20px;color:#66717e;font-size:8px;font-weight:900;">${i+1}</span>
        <span style="font-size:11px;font-weight:900;color:${pickColor};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${chatEsc(x.player)}</span>
      </div>
      <span style="font-size:14px;font-weight:950;color:${countColor};">${Number(x.count||0)}</span>
    </div>`;
  }).join("");
  ...
  el.innerHTML=`
    ...
    <div class="card" style="padding:16px;border:1px solid #343941;background:#0b0d10;">
      <div style="display:flex;align-items:end;justify-content:space-between;gap:10px;margin-bottom:5px;">
        <div>
          <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:1px;">WEEK ${week}</div>
          <div style="font-size:18px;font-weight:950;margin-top:2px;">Player Picks</div>
        </div>
        <div style="text-align:right;"><div style="font-size:7px;color:#7f8994;">TOTAL PICKS</div><div style="font-size:18px;font-weight:950;color:#31d07c;">${Number(bd.total||0)}</div></div>
      </div>
      <div>${rows||'<div class="muted" style="padding:12px 0;">No picks recorded for this week.</div>'}</div>
    </div>
    <div id="tdGamedayLiveMount">
      ${TD_GAMEDAY_BOARD
        ? tdRenderGamedayLive(TD_GAMEDAY_BOARD)
        : `<div class="card" style="margin-top:12px;padding:16px;border:1px solid #2b2f34;background:#08090a;">
            <div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:1px;">LIVE SURVIVOR BOARD</div>
            <div style="font-size:15px;font-weight:950;margin-top:4px;">Loading live games…</div>
            <div style="font-size:9px;color:#8f99a4;line-height:1.5;margin-top:6px;">Checking the read-only NFL Engine. Official grading remains commissioner-controlled.</div>
          </div>`}
    </div>`;

  tdWireGameday();

  if(!TD_GAMEDAY_BOARD&&!TD_GAMEDAY_LOADING){
    loadGamedayBoard().then(()=>renderSunday());
  }
}

ensureSundayAndLeagueUI();
...
document.querySelectorAll(".navbtn").forEach(b=>{
  b.onclick=async()=>{
    ...
    if(b.dataset.view==="sundayView"){
      renderSunday();
      loadGamedayBoard().then(()=>renderSunday());
      tdStartGamedayAutoRefresh();
    }
    if(b.dataset.view==="leagueView")renderLeague();
...
document.querySelector("#confirm").onclick=async()=>{
  ...
  try{
    // Immediate visual acknowledgement: no dead/silent wait after tapping.
    confirmBtn.disabled=true;
    const originalConfirmText=confirmBtn.textContent;
    confirmBtn.textContent="Saving…";
    showPickSavingOverlay(e.label,chosenName);
    ...
    // POST is no-cors, so its response body cannot prove success.
    // Independently read the exact pick back from the server before calling it saved.
    const receipt=await verifyPickReceipt(e.id,chosenId,weekAtSubmit);
    if(!receipt?.confirmed){
      hidePickSavingOverlay();
      throw new Error("TD Survivor could not verify this pick yet. Reopen the app and check this play before trying again.");
    }

    // Exact player has been read back from the server.
    try{
    ...
    showPickConfirmedOverlay(e.label,chosenName,weekAtSubmit);

    // IMPORTANT FOR MULTI-PLAY OWNERS:
    // Do not immediately launch refreshLive() here. refreshLive() performs a
    // full render() when its backend response arrives. If the owner switches
    // to Play 2 while that request is still in flight, the late render can
    // replace the Play 2 modal/button DOM mid-interaction and leave the screen
    // looking stuck on Saving/Loading even though the backend already saved it.
    //
    // The saved pick is already reflected in local state/cache above. The next
    // normal app refresh/reopen will reconcile with the backend.
    confirmBtn.disabled=false;
    confirmBtn.textContent=originalConfirmText;
  }catch(err){
    hidePickSavingOverlay();
    // This catch represents an actual submit or receipt-verification failure.
    const liveConfirm=document.querySelector("#confirm");
    if(liveConfirm){
      liveConfirm.disabled=false;
      liveConfirm.textContent="Confirm Pick";
    }
...
function showPickSavingOverlay(entryLabel,playerName){
  let overlay=document.querySelector("#tdPickSavingOverlay");
  if(!overlay){
    overlay=document.createElement("div");
    overlay.id="tdPickSavingOverlay";
  ...
  overlay.style.cssText=[
    ...
    "box-sizing:border-box","background:rgba(0,0,0,.86)",
    "backdrop-filter:blur(7px)"
  ].join(";");

  overlay.innerHTML=`
    <div style="width:min(390px,100%);padding:20px;border-radius:18px;border:1px solid #39424c;background:#0d1014;box-shadow:0 22px 70px #000;">
      <div style="font-size:9px;color:#ff6a00;font-weight:950;letter-spacing:1px;">SAVING YOUR PICK</div>
      <div style="font-size:25px;font-weight:950;color:#fff;margin-top:8px;line-height:1.05;">${playerName}</div>
      <div style="font-size:10px;color:#9aa4af;margin-top:5px;">${entryLabel}</div>

      <div style="display:flex;align-items:center;gap:10px;margin-top:18px;padding:12px;border-radius:12px;background:#090b0e;border:1px solid #242a31;">
        <div style="width:19px;height:19px;border:3px solid #434b55;border-top-color:#ff6a00;border-radius:50%;animation:tdPickSpin .7s linear infinite;"></div>
        <div>
          <div style="font-size:10px;font-weight:950;color:#fff;">Confirming with TD Survivor…</div>
          <div style="font-size:8px;color:#8d97a2;margin-top:2px;">Keep the app open until you see the green confirmation.</div>
        </div>
      </div>

      <div id="tdPickSavingNote" style="font-size:7.5px;color:#69737e;line-height:1.45;margin-top:10px;">
        Usually takes only a few seconds.
      </div>
    </div>
  `;

  if(!document.querySelector("#tdPickSpinStyle")){
    const st=document.createElement("style");
    st.id="tdPickSpinStyle";
    st.textContent="@keyframes tdPickSpin{to{transform:rotate(360deg)}}";
    document.head.appendChild(st);
  }

  clearTimeout(showPickSavingOverlay._slowTimer);
  showPickSavingOverlay._slowTimer=setTimeout(()=>{
    const note=document.querySelector("#tdPickSavingNote");
    if(note)note.innerHTML="<b style='color:#f0b45d;'>Still confirming…</b> Don't submit again or close the app yet.";
  },5000);
}

function hidePickSavingOverlay(){
  clearTimeout(showPickSavingOverlay._slowTimer);
  const overlay=document.querySelector("#tdPickSavingOverlay");
  if(overlay)overlay.style.display="none";
}

function showPickConfirmedOverlay(entryLabel,playerName,week){
  let overlay=document.querySelector("#tdPickSavingOverlay");
  if(!overlay){
    overlay=document.createElement("div");
    overlay.id="tdPickSavingOverlay";
  ...
  overlay.style.cssText=[
    "position:fixed","inset:0","z-index:20000","display:flex",
    "align-items:center","justify-content:center","padding:22px",
    "box-sizing:border-box","background:rgba(0,0,0,.88)",
    "backdrop-filter:blur(7px)"
  ].join(";");

  overlay.innerHTML=`
    <div style="width:min(390px,100%);padding:21px;border-radius:18px;border:1px solid #2b7652;background:#0b1711;box-shadow:0 22px 70px #000;">
      <div style="font-size:42px;line-height:1;">✅</div>
      <div style="font-size:10px;color:#42dc88;font-weight:950;letter-spacing:1px;margin-top:8px;">PICK CONFIRMED</div>
      <div style="font-size:25px;font-weight:950;color:#fff;margin-top:7px;line-height:1.05;">${playerName}</div>
      <div style="font-size:10px;color:#a5b4ab;margin-top:5px;">${entryLabel} • Week ${week}</div>
      <div style="font-size:9px;color:#d4ded8;margin-top:15px;padding:10px;border-radius:10px;background:#08110d;border:1px solid #244d39;">
        Saved to the league. You can safely close the app.
      </div>
      <button id="tdPickConfirmedDone" style="width:100%;margin-top:12px;padding:12px;border-radius:11px;border:1px solid #2b7652;background:#0e5b38;color:#fff;font-weight:950;">
        Done
      </button>
    </div>
  `;

  document.querySelector("#tdPickConfirmedDone").onclick=()=>hidePickSavingOverlay();
}

async function verifyPickReceipt(entryId,playerId,week){
  const delays=[0,350,650,1000,1500];
  let last=null;

  for(const delay of delays){
    if(delay)await new Promise(r=>setTimeout(r,delay));
    try{
      const r=await jsonp("pickReceipt",{
        token:OWNER_TOKEN,
      ...
      console.warn("Pick receipt check failed",err);
    }
  }
