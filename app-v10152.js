...
function renderBuybackBucket(){
  ...
  if(e.buybackUsed){
    ...
    value.style.fontSize="8.5px";
    value.style.letterSpacing="-.25px";
  }
}
...
function renderAdmin(){
  ...
  document.querySelector("#removeEntryAdmin").onclick=async()=>{
    ...
    const label=prompt(
      "Type the exact play name to remove:\\n\\n"+
      entriesSorted.slice(0,25).map(e=>"• "+e.label).join("\\n")
    );
    if(!label)return;

    const entry=entriesSorted.find(e=>String(e.label).toLowerCase()===String(label).trim().toLowerCase());
    if(!entry){alert("Play not found. Type the exact play name.");return;}

    if(!confirm(
      `REMOVE PLAY: ${entry.label}?\\n\\n`+
      `This permanently deletes this play AND all of its pick history.\\n\\n`+
      `The owner account will remain in the league.`
    ))return;

    const typed=prompt(`Final confirmation: type REMOVE ${entry.label}`);
    if(String(typed||"").trim()!==`REMOVE ${entry.label}`){
  ...
  document.querySelector("#removeOwnerAdmin").onclick=async()=>{
    const ownersSorted=[...(adminState.owners||[])].filter(o=>o.active!==false).sort((a,b)=>String(a.name).localeCompare(String(b.name)));
    if(!ownersSorted.length){alert("There are no people to remove.");return;}

    const name=prompt(
      "Type the exact person name to remove:\\n\\n"+
      ownersSorted.slice(0,25).map(o=>"• "+o.name).join("\\n")
    );
    if(!name)return;

    const owner=ownersSorted.find(o=>String(o.name).toLowerCase()===String(name).trim().toLowerCase());
    if(!owner){alert("Person not found. Type the exact name.");return;}

    const ownedEntries=(adminState.entries||[]).filter(e=>String(e.ownerId)===String(owner.id));
    if(!confirm(
      `REMOVE PERSON: ${owner.name}?\\n\\n`+
      `This permanently deletes their owner account, ${ownedEntries.length} play${ownedEntries.length===1?"":"s"}, and ALL pick history attached to those plays.\\n\\n`+
      `This cannot be undone.`
    ))return;

    const typed=prompt(`Final confirmation: type REMOVE ${owner.name}`);
    if(String(typed||"").trim()!==`REMOVE ${owner.name}`){
      alert("Removal cancelled.");
      return;
    }
