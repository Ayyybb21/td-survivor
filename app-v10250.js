...
function renderAdmin(){
  ...
  view.innerHTML=`
    ...
    <div class="admin-toolbar">
      ...
      <button id="autoNflGradeAdmin">⚡ Automatic NFL Grading: ${(adminState.autoNflGrading?.enabled)?"ON":"OFF"}</button>
  ...
  document.querySelector("#advanceWeekAdmin").onclick=async()=>{
    ...
    try{
      ...
      try{
        let fresh=null,lastFreshErr=null;
        for(let attempt=1;attempt<=3&&!fresh;attempt++){
          try{fresh=await jsonp("adminState",{adminToken:ADMIN_TOKEN});}
          catch(err){lastFreshErr=err;if(attempt<3)await new Promise(r=>setTimeout(r,450*attempt));}
        }
        if(!fresh)throw lastFreshErr||new Error("Could not load Week Recap data.");
        ...
        let posted=false,lastPostErr=null;
        for(let attempt=1;attempt<=3&&!posted;attempt++){
          try{
            ...
            const chatRes=await fetch(`${CHAT_URL}/functions/v1/chat-api`,{
              method:"POST",
              headers:{"Content-Type":"application/json",apikey:CHAT_KEY},
              body:JSON.stringify({action:"system_post",body:recap,systemProof:proof.systemProof})
            });
            const chatOut=await chatRes.json();
            if(!chatRes.ok||!chatOut.ok)throw new Error(chatOut.error||"Chat post failed.");
            posted=true;
          }catch(err){lastPostErr=err;if(attempt<3)await new Promise(r=>setTimeout(r,500*attempt));}
        }
        if(!posted)throw lastPostErr||new Error("Week Recap chat post failed.");
      }catch(chatErr){
    ...
    try{
      ...
      const grade=async(player,scored)=>{
        ...
        try{
          ...
          // V10.25: update the current board immediately; reconcile Sheets/NFL in background.
          const group=(TD_GAMEDAY_BOARD?.groups||[]).find(g=>slug(g.player)===slug(player));
          if(group){
            group.officialResult=scored?"TD":"No TD";
            (group.entries||[]).forEach(e=>e.officialResult=scored?"TD":"No TD");
          }
          try{localStorage.setItem("td_gameday_board",JSON.stringify(TD_GAMEDAY_BOARD||{}))}catch(_){}
...
Get those picks in!`;
  ...
  document.querySelector("#autoNflGradeAdmin").onclick=async()=>{
    const current=Boolean(adminState.autoNflGrading?.enabled);
    const next=!current;
    const msg=next
      ? "Turn Automatic NFL Grading ON?\n\nTD Survivor will automatically grade a player TD only after a rushing/receiving TD is confirmed, and No TD only after the game is FINAL with valid zero-TD stats. Uncertain data stays Pending. Manual commissioner overrides remain available."
      : "Turn Automatic NFL Grading OFF?\n\nLive NFL evidence will still display, but official results will require commissioner grading.";
    if(!confirm(msg))return;
    const btn=document.querySelector("#autoNflGradeAdmin");
    btn.disabled=true;
    try{
      await safeCommissionerPost("setAutoNflGrading",{adminToken:ADMIN_TOKEN,enabled:next});
      if(!adminState.autoNflGrading)adminState.autoNflGrading={};
      adminState.autoNflGrading.enabled=next;
      btn.textContent=`⚡ Automatic NFL Grading: ${next?"ON":"OFF"}`;
    }catch(err){alert("Automatic NFL Grading: "+err.message)}
...
async function loadGamedayBoard(force=false){
  ...
  // V10.25 instant paint: reuse the last known Gameday snapshot while the
  // backend refreshes/pre-warms centrally.
  if(!TD_GAMEDAY_BOARD){
    try{
      const saved=JSON.parse(localStorage.getItem("td_gameday_board")||"null");
      if(saved&&Number(saved.week)===Number(state.league.week))TD_GAMEDAY_BOARD=saved;
    }catch(_){}
  }
...
function tdRenderGamedayLive(board){
  ...
  return `
    <div class="card" style="margin-top:12px;padding:15px;border:1px solid #343941;background:#080a0c;">
      ...
      <div style="font-size:7px;color:#67717d;line-height:1.45;margin-top:9px;">NFL Engine updates automatically. Official results may be graded automatically when the safety rules are fully satisfied; commissioner overrides remain available.${checked?` • Checked ${checked}`:""}</div>
    </div>
    ${feed}
    ...
    <div id="tdGamedayLiveMount">
      ${TD_GAMEDAY_BOARD
        ...
        : `<div class="card" style="margin-top:12px;padding:16px;border:1px solid #2b2f34;background:#08090a;">
            ...
            <div style="font-size:9px;color:#8f99a4;line-height:1.5;margin-top:6px;">Loading the latest NFL snapshot. Safe automatic grading is available when enabled by the commissioner.</div>
          </div>`}
    </div>`;
