  ...
  document.querySelector("#advanceWeekAdmin").onclick=async()=>{
    ...
    try{
      ...
      try{
        ...
        const buybacksUsed=freshEntries.filter(e=>e.buybackUsed).length;
        const buybacksAvailableOut=freshEntries.filter(e=>e.status==="out"&&!e.buybackUsed).length;
        ...
        const recap=[
          ...
          `🔴 ${outNow} play${outNow===1?" is":"s are"} currently OUT.`,
          buybacksUsed ? `♻️ ${buybacksUsed} buyback${buybacksUsed===1?" has":"s have"} been used.` : "",
          buybacksAvailableOut ? `🟡 ${buybacksAvailableOut} OUT play${buybacksAvailableOut===1?" still has":"s still have"} a buyback available.` : "",
