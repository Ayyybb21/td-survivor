...
document.querySelector("#confirm").onclick=async()=>{
  ...
  try{
    ...
    // Update local state immediately from the acknowledged save.
    ...
    closeModal();
    saveCachedLiveState(state);
    render();

    // Confirmation is now independent of the slower full refresh.
    alert(`${e.label} Week ${weekAtSubmit} pick saved: ${chosenName}`);

    // Reconcile the rest of the league data quietly AFTER confirmation.
    // Do not await this; the user's saved-pick acknowledgement is complete.
    refreshLive().catch(err=>{
      console.warn("Background refresh after pick save failed",err);
    });

    // The modal is already closed, but restore in case this DOM node survives.
    confirmBtn.textContent=originalConfirmText;
  }catch(err){
    // Keep the modal open on failure so the owner can retry.
