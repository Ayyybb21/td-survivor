...
function renderAdmin(){
  ...
  document.querySelector("#setPickAdmin").onclick=()=>{
    ...
    panel.innerHTML=`
      ...
      <input id="setPickPlayerSearch" list="setPickPlayerList"
      ...
      <datalist id="setPickPlayerList">
        ${PLAYERS.map(p=>`<option value="${p.name}">${p.team} • ${p.position}</option>`).join("")}
      </datalist>
    ...
    entrySelect.onchange=()=>{
      ...
      if(e.currentPick){
        document.querySelector("#setPickPlayerSearch").value=e.currentPick;
      }else{
        document.querySelector("#setPickPlayerSearch").value="";
      }
    };

    document.querySelector("#saveSetPick").onclick=async()=>{
      const entryId=entrySelect.value;
      const typed=document.querySelector("#setPickPlayerSearch").value.trim();
      ...
      const player=PLAYERS.find(p=>p.name.toLowerCase()===typed.toLowerCase());
      if(!player){
        alert("Please choose a player from the NFL player list.");
        return;
      }
