...
function renderPickHero(){
  ...
  const weekCard=document.querySelector("#pickView .week-card") || document.querySelector("#pickView")?.children?.[0];
  if(!weekCard)return;

  let hero=document.querySelector("#tdDynamicHero");
  if(!hero){
    hero=document.createElement("div");
    hero.id="tdDynamicHero";
    hero.style.cssText="position:relative;margin:0 0 12px;padding:15px 78px 15px 15px;border-radius:16px;overflow:hidden;";
    weekCard.prepend(hero);
  }

  const isAlive=e.status==="alive";
  const hasPick=Boolean(pick?.player);
  const warning=isAlive&&!hasPick;
  const accent=warning?"#f4c430":hasPick?"#35d07f":"#ff7d88";
  const photo=hasPick
    ? (player?.photo || (pick.playerId?playerHeadshot(pick.playerId):""))
    : "";

  hero.style.border=`1px solid ${accent}88`;
  hero.style.background=warning
    ? "linear-gradient(110deg,#1d190b,#101a28 70%)"
    : hasPick
      ? "linear-gradient(110deg,#0b2119,#101a28 70%)"
      : "linear-gradient(110deg,#211116,#101a28 70%)";
  hero.style.boxShadow=`0 0 22px ${accent}18`;

  const title=warning
    ? `<span style="color:${accent}">Pick</span> your TD scorer`
    : hasPick
      ? `Your TD scorer: <span style="color:${accent}">${pick.player}</span>`
      : `TD scorer unavailable`;

  hero.innerHTML=`
    <div style="font-size:7px;font-weight:950;letter-spacing:1.2px;color:${accent};margin-bottom:7px;">WEEK ${state.league.week}</div>
    <div style="font-size:20px;font-weight:950;letter-spacing:-.6px;line-height:1.05;">${title}</div>
    <div style="font-size:9px;color:#9aacbf;line-height:1.4;margin-top:7px;">Rushing or receiving touchdowns only. Passing TDs do not count.</div>
    <div style="position:absolute;right:12px;top:50%;transform:translateY(-50%);width:58px;height:58px;border-radius:50%;border:2px solid ${accent};background:${accent}18;overflow:hidden;display:flex;align-items:center;justify-content:center;box-shadow:0 0 18px ${accent}44;">
      ${photo
        ? `<img src="${photo}" alt="" style="width:100%;height:100%;object-fit:cover;object-position:center top;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div style="display:none;width:100%;height:100%;align-items:center;justify-content:center;font-size:20px;font-weight:950;">${playerInitials(pick.player)}</div>`
        : `<div style="font-size:24px;opacity:.42;">👤</div>`}
    </div>
    ${warning?`<div style="display:inline-flex;margin-top:8px;padding:4px 7px;border-radius:999px;background:${accent};color:#101010;font-size:7px;font-weight:950;">⚠ NO PICK YET</div>`:""}
  `;

  // The new hero is the source of truth; hide the older static title/football
  // content while preserving the existing deadline/lock controls beneath it.
  [...weekCard.children].forEach(child=>{
    if(child===hero)return;
    const txt=String(child.textContent||"").trim();
    if(/Your TD scorer|Pick one player to score a rushing/i.test(txt) && !/Pick deadline/i.test(txt)){
...
function renderLeagueHistory(selectedId=null,mode="history"){
  ...
  if(mode==="history"){
    body.innerHTML=picks.length?picks.map(p=>`
      ...
      </div>`).join(""):'<div class="muted">No picks submitted yet.</div>';
  }else{
    body.innerHTML=`
