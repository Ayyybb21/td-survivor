...
async function loadNFLShadowHero(){
  ...
  if(!box||!badge)return;
  const entry=activeEntry(),pick=currentWeekPick(entry),player=playerByPick(pick);
  const team=String(player?.team||"").toUpperCase();
  if(!pick?.player||!team){badge.textContent="NO MATCH";box.textContent="Game data is not available for this pick yet.";return;}
  try{
    const r=await jsonp("nflShadowSchedule",{week:String(state.league.week)});
    const games=Array.isArray(r.games)?r.games:[];
    const g=games.find(x=>String(x.home||"").toUpperCase()===team||String(x.away||"").toUpperCase()===team);
    if(!g){badge.textContent="NO GAME";box.innerHTML=`No Week ${state.league.week} NFL game matched ${team} yet.`;return;}
    ...
    const score=hasScore?`<b style="color:#fff;">${g.away} ${g.awayScore}</b> &nbsp;—&nbsp; <b style="color:#fff;">${g.homeScore} ${g.home}</b>`:`<b style="color:#fff;">${g.away}</b> @ <b style="color:#fff;">${g.home}</b>`;
    const detail=s==="live"?[g.periodLabel,g.clock].filter(Boolean).join(" • "):s==="scheduled"?(g.kickoffLabel||"Kickoff time pending"):"Game complete";
    box.innerHTML=`<div>${score}</div><div style="margin-top:3px;color:${s==="live"?"#e7ebef":"#8f99a4"};">${detail}</div><div style="margin-top:6px;color:#89939d;">${pick.player}: <b style="color:#cdd3d9;">TD tracking not enabled yet</b></div>`;
  }catch(err){badge.textContent="UNAVAILABLE";box.textContent="NFL schedule data is temporarily unavailable.";console.warn("NFL shadow mode",err);}
}
