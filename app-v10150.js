...
(function installPrimeTimeExactOverrides(){
  ...
  style.textContent=`
    #chatView,#chatView>div,#chatMessages,#chatMessages+div{background:#060708!important;}
...
function renderAdmin(){
  ...
  document.querySelector("#inviteOwnerAdmin").onclick=async()=>{
    ...
    const ownersSorted=[...(adminState.owners||[])].sort((a,b)=>String(a.name).localeCompare(String(b.name)));
    if(!ownersSorted.length){
      alert("No participants found. Add a person first.");
      return;
    }
    ...
    panel.innerHTML=`
      <div style="font-size:9px;color:#ff7a1a;font-weight:950;letter-spacing:.9px;margin-bottom:4px;">INVITE / SHARE</div>
      <div style="font-size:8px;color:#929ca7;line-height:1.4;margin-bottom:10px;">
        Choose a participant to generate their private TD Survivor link.
        Anyone with that link can access that participant's plays, so send it only to them.
      </div>

      <select id="inviteOwnerSelect"
        style="width:100%;background:#090b0e;color:#fff;border:1px solid #343941;border-radius:10px;padding:11px;margin-bottom:9px;">
        <option value="">Select participant…</option>
        ${ownersSorted.map(o=>`<option value="${o.id}">${o.name}</option>`).join("")}
      </select>

      <button id="generateInviteLink" class="primary" style="width:100%;">Generate Private Invite Link</button>

      <div id="inviteResult" style="display:none;margin-top:11px;padding-top:11px;border-top:1px solid #292e35;">
        <div id="inviteOwnerName" style="font-size:10px;font-weight:950;color:#fff;margin-bottom:6px;"></div>
        <div id="inviteUrlDisplay"
        ...
        <div style="font-size:7.5px;color:#7f8994;line-height:1.45;margin-top:8px;">
          This private link signs the participant into their TD Survivor account. They can save the site to their Home Screen after opening it.
        </div>
      </div>
    `;
    ...
    document.querySelector("#generateInviteLink").onclick=async()=>{
      const select=document.querySelector("#inviteOwnerSelect");
      ...
      const btn=document.querySelector("#generateInviteLink");
      btn.disabled=true;
      ...
      try{
        ...
        if(!data?.token)throw new Error("Invite token was not returned.");

        const url=new URL(baseInviteUrl());
        url.searchParams.set("token",data.token);

        currentInvite={
          name:data.name||ownersSorted.find(o=>String(o.id)===String(ownerId))?.name||"Participant",
          url:url.toString()
        };

        document.querySelector("#inviteOwnerName").textContent=`Private link for ${currentInvite.name}`;
        document.querySelector("#inviteUrlDisplay").textContent=currentInvite.url;
        document.querySelector("#inviteResult").style.display="block";
      }catch(err){
        alert("Could not generate invite link: "+err.message);
      }finally{
        btn.disabled=false;
        btn.textContent="Generate Private Invite Link";
      }
    };

    document.querySelector("#copyInviteLink").onclick=async()=>{
      if(!currentInvite)return;
      try{
        await copyTextFallback(currentInvite.url);
        const b=document.querySelector("#copyInviteLink");
    ...
    document.querySelector("#shareInviteLink").onclick=async()=>{
      if(!currentInvite)return;
      const text=`You're invited to TD Survivor.\n\nOpen your private link below to access your play${(adminState.entries||[]).filter(e=>e.ownerId===document.querySelector("#inviteOwnerSelect").value).length>1?"s":""}:\n${currentInvite.url}`;

      if(navigator.share){
        try{
          await navigator.share({
            title:"TD Survivor Invite",
            text
          });
          return;
        }catch(err){
