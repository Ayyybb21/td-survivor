...
Get those picks in!`;
  ...
  document.querySelector("#freshLeagueAdmin").onclick=()=>{
    ...
    document.querySelector("#runFreshLeague").onclick=async()=>{
      ...
      try{
        const result=await post("freshLeagueReset",{
          adminToken:ADMIN_TOKEN,
          keepOwnerId,
          startWeek,
          deadlineLabel,
        ...
        btn.textContent="✅ Fresh league created";
        alert(
          `FRESH LEAGUE READY ✅\n\n`+
          `${result.commissioner?.name||"Commissioner"} is the only participant.\n`+
          `Owners: ${result.ownersRemaining}\n`+
          `Plays: ${result.entriesRemaining}\n`+
          `Old picks: ${result.picksRemaining}\n`+
          `Week: ${result.startWeek}\n\n`+
          `The old league invite is invalid. Generate/share the new invite from Invite / Share.`
        );
