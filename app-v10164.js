...
async function renderNotificationCard(){
  ...
  card.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;"><div><div style="font-size:8px;color:#ff7a1a;font-weight:950;letter-spacing:.8px;">NOTIFICATIONS</div><div style="font-size:12px;font-weight:950;margin-top:2px;">${enabled?"Reminders are ON":"Never miss a pick"}</div><div style="font-size:8px;color:#929ca7;line-height:1.45;margin-top:4px;">${enabled?"Missing picks, locks and commissioner updates can reach this phone.":isiOS&&!standalone?"Open TD Survivor from your Home Screen to enable push reminders.":"Enable phone alerts for important TD Survivor reminders."}</div></div><button id="tdPushToggle" style="flex:0 0 auto;padding:9px 10px;border-radius:9px;border:1px solid ${enabled?"#315d47":"#a83d00"};background:${enabled?"#0d2118":"#211109"};color:${enabled?"#31d07c":"#ff7a1a"};font-weight:950;">${enabled?"ON":"Enable"}</button></div>`;
...
function renderAdmin(){
  ...
  document.querySelector("#deadlineAdmin").onclick=async()=>{
    const current=String(state.league.deadline||adminState.league.deadline_label||"Thu • 8:15 PM ET");

    const deadline=prompt(
      "CHANGE PICK DEADLINE\n\n"+
      "You can change this at any time during the current week.\n\n"+
      "Format example:\nThu • 8:15 PM ET\nWed • 4:30 PM ET\n\n"+
      "New deadline:",
      current
    );
    ...
    // Keep the format predictable because Smart Reminders reads this label.
    const valid=/\b(Sun|Mon|Tue|Wed|Thu|Fri|Sat)\b.*\b\d{1,2}:\d{2}\s*(AM|PM)\s*ET\b/i.test(clean);
    if(!valid){
      alert(
        "Please use this format:\n\n"+
        "Thu • 8:15 PM ET\n\n"+
        "The day and ET time are used by Smart Reminders."
      );
      return;
    }
    ...
    const btn=document.querySelector("#deadlineAdmin");
    btn.disabled=true;
    ...
    try{
      ...
      // Tell the league immediately. Chat failure does not undo the saved deadline.
      try{
      ...
      btn.textContent="✅ Deadline Updated";
      setTimeout(()=>window.location.reload(),700);
    }catch(err){
      btn.disabled=false;
      btn.textContent="⏰ Change Deadline";
